...
program define ROCcomp, sclass         /* Dependent (correlated) ROC curves */
	...
	local j 2
	while  `j'<=`bymax'+1 {
		qui su ``j'' if `touse', meanonly
		if r(min) == r(max) {
			noi di in red "variable ``j'' does not vary"
			exit 2000
		}
		local C`i'="``j''"
		local i=`i'+1
	...
	while `i' <= `bymax' {
		...
		*/ `id' `x' `N' `area' `se' `lb' `ub'
		...
		qui `vv' roctab `D' `C' `wt' , level(`level') `useado'
		qui replace `N'=r(N)
		...
		*/ `lb' `ub' `D' `C' `wv' `id' `order'
		...
		qui gen double `v01'=.
		qui gen double `v10'=.
		COMparea `D' `C' `wv' `na' `nn' `area' `v01' `v10' `useado'
		rename `v01' v01`i'
		rename `v10' v10`i'
		rename `wv' wv
...
*/ `"`spaces'[`=strsubdp("`level'")'% Conf. Interval]"' _n "{hline 73}"
		...
		local i 1
		while `i' <= `bymax' {
        		di in ye `bygrp'[`i'] _col(15) in yel %8.0fc `N'[`i'] /*
...
prog def COMparea, sclass
	args D C wv na nn area v01 v10 useado
	tempvar Phi
	qui gen double `Phi'=.
	sort `D' `C'
...
void _comparea_wrk(string scalar Cs, Ds, wvs, Phis) {

	real scalar i, N, ix1, ix2, x
	...
	while (i <= N) {		
		...
		if (D[i]==1) {
			...
			phi = sum(phi)
			Phi[|ix1,1 \ ix2,1|] = J(ix2-ix1+1,1,phi)
		}
		else {
