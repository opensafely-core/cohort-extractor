...
prog def rocfit, eclass sort
	...
	if replay() {
		...
		*/ MLOpt(string) NOLOg LOg CONTinuous(string) Generate(string) * ]
		...
		mlopts options, `options' `log' `nolog'
		...
		*noi di in red "`zv'"
		...
		qui mat `bc0' = ( `a0', `b0')   
		if _caller() < 15 {
			mat colnames `bc0' = a:_cons b:_cons
		}
		else {
			mat colnames `bc0' = a:_cons /b
		}
		...
		if `coef'<0 {
			...
			if _caller() < 15 {
				...
				qui mat colnames `telm' = a:_cons /b
			}
			local j 1
			local i=`tel'
			while `i' >2 {
