*! version 6.5.3  30jan2014
program define strate, rclass sortpreserve
	version 6.0, missing
	if _caller() < 8 {
		strate_7 `0'

		if `"`r(method)'"'=="jackknife" {
			return hidden scalar flag = r(flag)
...
quietly {
	...
	if "`jack'"!="" {
		...
		if "`ev'"!="" {
			...
			by `touse' `ev': /*
				*/ gen byte `NON' = sum(`nonzero') if `touse'
...
program define jckknife_note
	di as smcl "{p 0 6 0 79}Note: Jackknife confidence intervals "	     ///
