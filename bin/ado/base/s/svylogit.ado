...
program define svylogit
	...
	/* When here, logit has just been called.
...
program define Syntax, sclass
	sret clear
	sret local title    "Survey logistic regression"
	sret local cmd      "logit"
	sret local k_depvar "1"
	sret local dopts    "OR"  /* special display option */
...
program define HowMany, rclass
	...
	ret scalar cmdcando = 1 /* logit can compute it */
end
...
program define Footnote
	...
	di as txt "Note: `nfail' `failure' and `nsucc' " /*
