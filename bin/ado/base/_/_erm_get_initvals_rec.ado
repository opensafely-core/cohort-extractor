...
program _erm_get_initvals_rec, rclass
...
if "`ermcmd'" == "eregress" {
_erm_regress_getsv `depvar' `indepvars' `wexp', `constant' ///
	touse(`ttouse') storeresidual(`resid`j'') `extraopts'
}
else if "`ermcmd'" == "eintreg" {
_erm_intreg_getsv `indepvars' `wexp', ///
	ll(`depvarl') ul(`depvaru') `offset' `constant' ///
	touse(`ttouse') storeresidual(`resid`j'') `extraopts' ///
	depvarname(`depvarl')
}
else if "`ermcmd'" == "eprobit" {
_erm_probit_getsv `depvar' `indepvars' `wexp', `offset' `constant' ///
	touse(`ttouse') storeresidual(`resid`j'') `extraopts'
}
else if "`ermcmd'" == "eoprobit" {
_erm_oprobit_getsv `depvar' `indepvars' `wexp', `offset' `constant' ///
	touse(`ttouse') storeresidual(`resid`j'') `extraopts'
