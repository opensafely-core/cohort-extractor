...
program _eregress, eclass byable(recall) sortpreserve
	version 16.0
	syntax varlist(fv numeric ts) [if] [in] 			/*
			...
			*/ htitle(string)	/*UNDOCUMENTED*/	/*
			*/ NORECORRelation	/*UNDOCUMENTED*/	/*
	...
	if "`re'" == "" {
		local cmdline eregress `0'
	}
	else {
		local cmdline xteregress `0'
	}
	...
	if (`"`select'"' != "") {
		...
		if r(N) == 0  & "`htitle'" == "" {
			...
			di as error ///
			"{p 0 4 2}no non-selected observations{p_end}"
			exit 498
		}
	...
	local dvlist `depvar'`depvarl' `tsel_depvar'`sel_depvar' 	///
			...
			`endog_depvars'
	local intdvlist `tsel_depvar' `sel_depvar' `tr_depvar' ///
			`bendog_depvars' `oendog_depvars'
	...
	if "`norecorrelation'" != "" {
		// xtheckman
		constraint free 
		local constraintf `r(free)'
		constraint define `constraintf' ///
	_b[/corr(`sel_depvar'[`repanvar'],`depvar'[`repanvar'])] = 0
		AddConstraint, `constraints' free(`constraintf')
		local constraints `r(constraints)'
	}
	if ("`tr_depvar'" != "" | "`sel_depvar'" != ""	/// 
		...
		if "`re'" == "" {
			capture qui regress `depvar' `indepvars'	/// 
				if `touse' `wexp',			///
				`constant' notable noheader
			local rc = _rc
			if `rc' {
				if `rc' != 1 {
					di as error ///
			...
			tempname ibigB
			matrix `ibigB' = e(b)
			matrix coleq `ibigB' = "`depvar'"
			local f: colfullnames `ibigB'
			local s = e(rmse)^2
			matrix `ibigB'= (`ibigB',`s')
			local f `f' /:v_o
			...
			capture qui xtintreg `depvar' `depvar' `indepvars' /// 
				if `touse' `wexp',	re		///
				`constant' iter(0)
			local rc = _rc
			if `rc' {
	...
	nobreak {
		if "`has_pocovariance'" == "" {
			...
			if "`re'" == "" {
				local evaluator _eregress_lf()
			}
			else {
				if "`reintmethod'" == "mvaghermite" {
					local evaluator _eregress_lf_re_agh()
				}
				else {
					local evaluator _eregress_lf_re_re1()
				}
			}
		}
		else {
			mata: _eregress_treat_init(`pocovariance_n',	///
			 	"inits",				///
				"`depvar'","`sel_depvar'",		///
				"`tr_depvar'",				///
				"`pocovariance_depvar'",		///
				...
				"`adaptlog'","`relistmat'")
			if "`re'" == "" {
				local evaluator _eregress_tr_lf()
			}
			else {
				if "`reintmethod'" == "mvaghermite" {
					local evaluator _eregress_tr_lf_re_agh()
				}
				else {
					local evaluator _eregress_tr_lf_re_re1()
				}
			}
		}
		if "`re'" != ""	{
		...
		if "`re'" == "" {
			ereturn local cmd eregress
		}
		else {
			if "`htitle'" == "" {
				ereturn local cmd xteregress
			}
			else {
				ereturn local cmd xtheckman
			}
		...
		ereturn local marginsok default Mean Pr ///
			POMean xb te tet e(passthru) ///
			Pre(passthru) YStar(passthru) ///
			total STRuctural fix(passthru) ///
			base(passthru) TARGet(passthru)	///
		...
		ereturn hidden local pocovariance `has_pocovariance'
...
program AddConstraint, rclass
	syntax, [constraints(string) free(string)]
	if `"`constraints'"' == "" {
		local constraints constraints(`free')
	}
	else {
		local constraints constraints(`constraints' `free')
	}
	return local constraints `constraints'
end
