...
capture noi `nb' {
	...
	if "`catlist'" != "" {
		forvalues i = 1/`npotreat' {
			// catcond1 is mu
		mata:_erm_truncnorm_expmean_help("`outvar'",	///
			"`lowerlist'","`upperlist'", 			///
			"`lowerindlist'","`upperindlist'",		///
			"`condvarfull`i''",`intpoints',	///
			`intpoints3',"`touse`i''")
		qui replace `outvar' = `outvar'*exp(`catcond1') if `touse`i''
		}	
	}
	else {
		forvalues i = 1/`npotreat' {
			local sig`i' (sqrt(`condvarfull`i''[1,1]))
			qui replace `outvar' = exp(`catcond1'+	///
				.5*`condvarfull`i''[1,1]) if `touse`i''
		}
	}
