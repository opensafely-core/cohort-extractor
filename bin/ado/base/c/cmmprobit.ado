...
program cmmprobit, eclass byable(onecall) prop(cm)
	version 16
...
di as error "{p_end}"
	...
	if (   "`origpanelvar'" != ""				    /// 
		...
		_cm_vcepanel `origpanelvar', cmd(cmmprobit) `vce'
		local vce `"`s(vce)'"'
	}

	`BY' asmprobit `varlist' [`weight'`exp'] `if' `in', ///
		case(`caseid')                    	    ///
		alternatives(`altvar')            	    ///
		`altwise'				    ///
		`vce'					    ///
	...
	ereturn hidden local case   `caseid'  // asmprobit's name
	ereturn local        caseid `caseid'  // cmset's name
	
	local cmdline : list retokenize cmdline
	ereturn local cmdline `"cmmprobit `cmdline'"'
