...
{viewerjumpto "Syntax" "cmmprobit##syntax"}{...}
{viewerjumpto "Menu" "cmmprobit##menu"}{...}
{viewerjumpto "Description" "cmmprobit##description"}{...}
{viewerjumpto "Links to PDF documentation" "cmmprobit##linkspdf"}{...}
{viewerjumpto "Options" "cmmprobit##options"}{...}
{viewerjumpto "Examples" "cmmprobit##examples"}{...}
{viewerjumpto "Stored results" "cmmprobit##results"}{...}
{p2colset 1 19 21 2}{...}
{p2col:{bf:[CM] cmmprobit} {hline 2}}Multinomial probit choice model{p_end}
{p2col:}({mansection CM cmmprobit:View complete PDF manual entry}){p_end}
{p2colreset}{...}
...
{syntab:Model 2}
{synopt :{opth corr:elation(cmmprobit##correlation:correlation)}}correlation structure of the utility errors{p_end}
{synopt :{opth std:dev(cmmprobit##stddev:stddev)}}variance structure of the utility errors{p_end}
...
{synopt :{it:{help cmmprobit##display_options:display_options}}}control
INCLUDE help shortdes-displayoptall

{syntab:Integration}
{synopt :{opth intm:ethod(cmmprobit##seqtype:seqtype)}}type of quasi-uniform or pseudo-uniform sequence{p_end}
...
{synopt :{cmdab:ints:eed(}{help cmmprobit##code:{it:code}}{c |}{it:#}{cmd:)}}pseudo-uniform random-number seed{p_end}
...
{syntab:Maximization}
{synopt :{it:{help cmmprobit##maximize_options:maximize_options}}}control the maximization process; seldom used{p_end}
...
{p 4 6 2}
You must {cmd:cmset} your data before using {cmd:cmmprobit}; see
...
{p 4 6 2}
See {manhelp cmmprobit_postestimation CM:cmmprobit postestimation} for
...
{phang}
{bf:Statistics > Choice models > Multinomial probit model}
...
{pstd}
{cmd:cmmprobit} fits a multinomial probit (MNP) choice model that relaxes the
independence of irrelevant alternatives (IIA) property that is characteristic
of the {helpb cmclogit} choice model and that is assumed by the MNP model fit
by {helpb mprobit}.

{pstd}
The command requires multiple observations for each case (representing one
individual or decision maker), where each observation represents an alternative
that may be chosen.  {cmd:cmmprobit} allows two types of independent variables:
...
{title:Links to PDF documentation}
        ...
        {mansection CM cmmprobitQuickstart:Quick start}

        {mansection CM cmmprobitRemarksandexamples:Remarks and examples}

        {mansection CM cmmprobitMethodsandformulas:Methods and formulas}
...
correlations with all other utility errors are set to zero.  The default is
the first alternative when sorted.  If a {cmd:fixed} or {cmd:pattern} matrix
is given in the {cmd:stddev()} and {cmd:correlation()} options, the
...
{phang}
{cmd:noconstant} suppresses the J - 1 alternative-specific constant terms.

{phang}
{cmd:altwise} specifies that alternativewise deletion be used when
omitting observations because of missing values in your variables.  The
default is to use casewise deletion; that is, the entire group of observations
...
handled alternativewise regardless of whether {cmd:altwise} is specified.
...
{cmd:correlation()}.  A {bind:{it:#} x J} (or {bind:{it:#} x J - 1}) matrix,
{bf:C}, is used to factor the covariance matrix as {bind:I + {bf:C}'{bf:C}},
where I is the identity matrix of dimension J (or J - 1).  The column dimension
of {bf:C} depends on whether the covariance is structural or differenced.  The
...
{phang}
{opt structural} requests the J x J structural covariance
parameterization instead of the default J - 1 x J - 1 differenced
...
{pmore}
This option has no effect if {opt structural} is not specified because the
default differenced covariance estimates have no interesting
interpretation as correlations and standard deviations.  {opt notransform}
also has no effect if the {opt correlation()} and {opt stddev()} options are
...
{phang}
{opt intpoints(#)} specifies the number of points to use in the Monte Carlo
integration.  If option {cmd:intmethod(hammersley)} or {cmd:intmethod(halton)}
is used, the default is {bind:500 + floor(2.5 sqrt[N_c{ln(k + 5) + v}])},
where N_c is the number of cases, k is the number of coefficients in the
model, and v is the number of covariance parameters.  If
{cmd:intmethod(random)} is used, the number of points is the above times 2.
...
draw for the J - 1 vector uniform-random variables, {bf:x}, is 1 - {bf:x}.

{phang}
{opt nopivot} turns off integration interval pivoting.  By default, 
{opt cmmprobit} will pivot the wider intervals of integration to the interior
...
points are used, resulting in a non-positive-definite Hessian.  {cmd:cmmprobit}
uses the Broyden-Fletcher-Goldfarb-Shanno optimization algorithm, by default,
...
be used for the initial {it:#} optimization steps.  This option is the only
way to use the BHHH algorithm along with other optimization techniques.  The
algorithm switching feature of {cmd:ml}'s {cmd:technique()} option cannot
include {cmd:bhhh}.

{phang}
{cmd:favor(speed{c |}space)} instructs {cmd:cmmprobit} to favor either
...
specifies many integration points, {opt intpoints(#)}.  When favoring space,
...
the estimates computed using {cmd:intmethod(random)} will vary slightly
between {cmd:favor(speed)} and {cmd:favor(space)}.  This is because the
uniform sequences will not be identical, even when initiating the sequences
using the same uniform seed, {cmd:intseed(}{it:code}{c |}{it:#}{cmd:)}.  For
{cmd:favor(speed)}, {cmd:ncase} blocks of {opt intpoints(#)} x J - 2 uniform
points are generated, where J is the maximum number of alternatives.  For
{cmd:favor(space)}, the column dimension of the matrices of points varies with
the number of alternatives that each case has.
...
convergence with {cmd:cmmprobit}:
{cmd:difficult},
...
{opt bhhh} cannot be one of them.  To use the BHHH 
...
{pstd}
The following options are available with {cmd:cmmprobit} but are not shown in
the dialog box:
...
{pstd}Fit multinomial probit choice model{p_end}
...
{pstd}
{cmd:cmmprobit} stores the following in {cmd:e()}:
...
{synopt :{cmd:e(alt_casevars)}}indicators for estimated case-specific coefficients -- {cmd:e(k_alt)} x {cmd:e(k_casevars)}{p_end}
