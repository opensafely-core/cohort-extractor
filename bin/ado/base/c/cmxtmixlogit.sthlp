...
{viewerjumpto "Syntax" "cmxtmixlogit##syntax"}{...}
{viewerjumpto "Menu" "cmxtmixlogit##menu"}{...}
{viewerjumpto "Description" "cmxtmixlogit##description"}{...}
{viewerjumpto "Links to PDF documentation" "cmxtmixlogit##linkspdf"}{...}
{viewerjumpto "Options" "cmxtmixlogit##options"}{...}
{viewerjumpto "Examples" "cmxtmixlogit##examples"}{...}
{viewerjumpto "Stored results" "cmxtmixlogit##results"}{...}
{p2colset 1 22 24 2}{...}
{p2col:{bf:[CM] cmxtmixlogit} {hline 2}}Panel-data mixed logit choice
model{p_end}
{p2col:}({mansection CM cmxtmixlogit:View complete PDF manual entry}){p_end}
{p2colreset}{...}
...
{synoptline}
{syntab:Model}
{synopt :{opth casev:ars(varlist)}}choice set-specific variables{p_end}
{synopt :{cmdab:r:andom(}{varlist}[{cmd:,} {help cmxtmixlogit##distribution:{it:distribution}}]{cmd:)}}specify variables that are to have random coefficients and the coefficients' distribution{p_end}
{synopt :{opth corr:metric(cmxtmixlogit##metric:metric)}}correlation metric for correlated random coefficients{p_end}
...
{synopt :{it:{help cmxtmixlogit##display_options:display_options}}}control
INCLUDE help shortdes-displayoptall

{syntab:Integration}
{synopt :{opth intm:ethod(cmxtmixlogit##seqspec:seqspec)}}specify point set for Monte Carlo integration{p_end}
...
{syntab:Maximization}
{synopt :{it:{help cmxtmixlogit##maximize_options:maximize_options}}}control the maximization process; seldom used{p_end}
...
{synopt :{opt corr:elated}}correlated Gaussian-distributed random coefficients{p_end}
...
{p 4 6 2}
You must {cmd:cmset} your data before using {cmd:cmxtmixlogit};
see {manhelp cmset CM}.{p_end}
{p 4 6 2}
{it:indepvars} and {it:varlist} may contain factor variables and
...
{p 4 6 2}
See {manhelp cmxtmixlogit_postestimation CM:cmxtmixlogit postestimation} for
...
{phang}
{bf:Statistics > Choice models > Panel-data mixed logit model}
...
{pstd}
{cmd:cmxtmixlogit} fits a mixed logit model (MLM) to choice data in which
decision makers make repeated choices, typically at different time periods.
Data with repeated cases from the same units are also referred to as panel
data.  Panel-data mixed logit models use random coefficients to model the
correlation of choices across alternatives.  The random coefficients are on
variables that vary across alternatives (and may vary over individuals and
choice sets as well), known as alternative-specific variables.

{pstd}
The correlation of choices across alternatives relaxes the independence of
irrelevant alternatives (IIA) assumption required by the conventional
multinomial logit model, fit by {helpb mlogit}, and the choice-model
conditional logit model, fit by {helpb cmclogit}.

{pstd}
In the context of panel-data applications, the MLM models the probability of
selecting each alternative for each time period rather than modeling a single
probability for selecting each alternative, as in the case of cross-sectional
data.  Mixed logit models for cross-sectional data are fit by
{helpb cmmixlogit}.
...
{title:Links to PDF documentation}
        ...
        {mansection CM cmxtmixlogitQuickstart:Quick start}

        {mansection CM cmxtmixlogitRemarksandexamples:Remarks and examples}

        {mansection CM cmxtmixlogitMethodsandformulas:Methods and formulas}
...
constant for each case, that is, choice set.  If there are a maximum of A 
alternatives, there will be A - 1 sets of coefficients associated 
with {opt casevars()}.

{phang}
{cmd:random(}{varlist} [{cmd:,} {help cmxtmixlogit##distribution:{it:distribution}}]{cmd:)}
...
{phang}
{opth corrmetric:(cmxtmixlogit##metric:metric)}
...
{phang}
{cmd:intmethod(}{help cmxtmixlogit##seqtype:{it:seqtype}}[{cmd:,} {cmd:antithetics}{c |}{cmd:mantithetics}]{cmd:)} 
...
{mansection CM cmxtmixlogitMethodsandformulas:{it:Methods and formulas}}
in {bf:[CM] cmxtmixlogit}.

{phang}
{opt intpoints(#)} specifies the number of points to use in the
Monte Carlo integration.  The default number of points is a function of model
complexity and integration method.  If {cmd:intmethod(hammersley)} or
{cmd:intmethod(halton)} is used, the default is 
{bind:500 + floor(2.5 sqrt[N_c{ln(r + 5) + v}])}, where N_c is the number of
panels, r is the number of random coefficients in the model, and v is the
number of variance parameters.  If {cmd:intmethod(hammersley, mantithetics)}
or {cmd:intmethod(halton, mantithetics)} is used, the number of integration
points is {bind:250 + floor(0.5 sqrt[N_c{ln(r + 5) + v}])}.  If
{cmd:intmethod(random)} is used, the number of points is twice the number of
points used by {cmd:intmethod(hammersley)} and {cmd:intmethod(halton)}.
...
{cmd:intmethod(random)}.  {it:#} must be an integer between 0 and 2^{31} - 1.
The default is to use the current seed value from Stata's uniform
random-number generator; see {helpb set seed:[R] set seed}.

{phang}
{cmd:favor(speed{c |}space)} instructs {cmd:cmxtmixlogit} to favor either
...
{pstd}
The following options are available with {cmd:cmxtmixlogit} but are not shown
in the dialog box:
...
{pstd}Same as above, but include case-specific covariates {cmd:age} and {cmd:income}{p_end}
...
than coefficients{p_end}
{phang2}{cmd:. cmxtmixlogit, or}{p_end}
...
{pstd}
{cmd:cmxtmixlogit} stores the following in {cmd:e()}:

{synoptset 25 tabbed}{...}
{p2col 5 25 29 2: Scalars}{p_end}
{synopt :{cmd:e(N)}}number of observations{p_end}
{synopt :{cmd:e(N_case)}}number of cases{p_end}
{synopt :{cmd:e(N_panel)}}number of panels{p_end}
...
{synopt :{cmd:e(t_avg)}}average number of time points per panel{p_end}
{synopt :{cmd:e(t_min)}}minimum number of time points per panel{p_end}
{synopt :{cmd:e(t_max)}}maximum number of time points per panel{p_end}
...
{synopt :{cmd:e(corrmetric)}}correlation metric for correlated random
coefficients{p_end}
