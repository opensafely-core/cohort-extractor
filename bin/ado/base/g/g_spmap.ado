...
*! -g_spmap-: Visualization of spatial data                                    
...
if `EXIST' {
   ...
   g_spmap_psl, l(`fcolor') m(`MAX') o({bf:{ul:fc}olor()}) d(`fcolor_d')
   local fcolor `"`s(pl)'"'
}
...
if `EXIST' {
   ...
   g_spmap_psl, l(`ocolor') m(`MAX') o({bf:{ul:oc}olor()}) d(`ocolor_d')
   local ocolor `"`s(pl)'"'
}

/* Parse option osize() */
g_spmap_psl, l(`osize') m(`MAX') o({bf:{ul:os}ize()}) d(`osize_d')
local osize `"`s(pl)'"'

/* Parse option opattern() */
g_spmap_psl, l(`opattern') m(`MAX') o({bf:{ul:op}attern()}) d(`opattern_d')
local opattern `"`s(pl)'"'

/* Parse option oalign() */
g_spmap_psl, l(`oalign') m(`MAX') o({bf:{ul:oa}lign()}) d(`oalign_d')
local oalign `"`s(pl)'"'
...
if (`"`scalebar'"' != "") {
   qui g_spmap_scalebar, `scalebar' xmin(`xmin') xmax(`xmax')   ///
                                  ymin(`ymin') ymax(`ymax')
...
if `EMBPOL' {
   ...
   local GRAPH `"`GRAPH' lc("`OC'") lw("`OS'") lp("`OP'") la("`OA'")) "'
   forval i = 1/`NC' {
   ...
   forval i = 1/`NC' {
      ...
      local GRAPH `"`GRAPH' lw("`OS'") lp("`OP'") la("`OA'")) "'
   }
}
