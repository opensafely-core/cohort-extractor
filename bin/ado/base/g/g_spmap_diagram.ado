*! version 1.0.1  10jul2017
*! -g_spmap_diagram-: Auxiliary program for -g_spmap-                          
...
program g_spmap_diagram, rclass
version 9.2
...
if `EXIST' {
   ...
   g_spmap_psl, l(`fcolor') m(`NG') o({bf:{ul:fc}olor()}) d(`fcolor_d')
   local fcolor `"`s(pl)'"'
}

g_spmap_psl, l(`oalign') m(`NG') o({bf:{ul:oa}liagn()}) d(`oalign_d')
local oalign `"`s(pl)'"'
...
local GRAPH `"`GRAPH' cmissing(n) fc("white") lc("white") lw("none") la(center)) "'
forval i = 1/`NG' {
   ...
   local GRAPH `"`GRAPH' lw("none") la("`OA'")) "'
}
...
local GRAPH `"`GRAPH' cmissing(n) fc("none") lc("`OC'") lw("`OS'") la("`OA'")) "'
...
if `EXIST' {
   ...
   g_spmap_psl, l(`fcolor') m(`NVAR') o({bf:{ul:fc}olor()}) d(`fcolor_d')
   local fcolor `"`s(pl)'"'
}
...
if `EXIST' {
   ...
   g_spmap_psl, l(`ocolor') m(`NVAR') o({bf:{ul:oc}olor()}) d(`ocolor_d')
   local ocolor `"`s(pl)'"'
}

/* Parse option osize() */
g_spmap_psl, l(`osize') m(`NVAR') o({bf:{ul:os}ize()}) d(`osize_d')
local osize `"`s(pl)'"'

/* Parse option oalign() */
g_spmap_psl, l(`oalign') m(`NVAR') o({bf:{ul:os}ize()}) d(`oalign_d')
local oalign `"`s(pl)'"'
