...
*! -g_spmap_polygon-: Auxiliary program for -g_spmap-                              
...
program g_spmap_polygon, rclass
version 9.2
...
if _rc {
   di as err "{p}Problem with option {bf:{ul:pol}ygon()}: file "       ///
             "{bf:`data'} specified in suboption {bf:{ul:d}ata()} "    ///
             "is not a valid {help grmap##sd_polygon:{it:polygon}} "   ///
             "dataset {p_end}"
   exit 198
}
...
if `EXIST' {
   ...
   g_spmap_psl, l(`ocolor') m(`NG') o({bf:{ul:oc}olor()}) d(`ocolor_d')
   local ocolor `"`s(pl)'"'
}

/* Parse option osize() */
g_spmap_psl, l(`osize') m(`NG') o({bf:{ul:os}ize()}) d(`osize_d')
local osize `"`s(pl)'"'

/* Parse option opattern() */
g_spmap_psl, l(`opattern') m(`NG') o({bf:{ul:op}attern()}) d(`opattern_d')
local opattern `"`s(pl)'"'

/* Parse option oalign() */
g_spmap_psl, l(`oalign') m(`NG') o({bf:{ul:op}attern()}) d(`oalign_d')
local oalign `"`s(pl)'"'
