*! version 1.0.0  17mar2017
*! -g_spmap_arrow-: Auxiliary program for -g_spmap-                                
...
if _rc {
   di as err "{p}Problem with option {bf:{ul:arr}ow()}: file "        ///
             "{bf:`data'} specified in suboption {bf:{ul:d}ata()} "   ///
             "is not a valid {help grmap##sd_arrow:{it:arrow}} "      ///
             "dataset {p_end}"
   exit 198
}
...
g_spmap_psl, l(`direction') m(`NG') o({bf:{ul:d}irection()}) d(`direction_d')
local direction `"`s(pl)'"'

/* Parse option hsize() */
g_spmap_psl, l(`hsize') m(`NG') o({bf:{ul:hsi}ze()}) d(`hsize_d')
local hsize `"`s(pl)'"'

/* Parse option hangle() */
g_spmap_psl, l(`hangle') m(`NG') o({bf:{ul:han}gle()}) d(`hangle_d')
local hangle `"`s(pl)'"'

/* Parse option hbarbsize() */
g_spmap_psl, l(`hbarbsize') m(`NG') o({bf:{ul:hba}rbsize()}) d(`hbarbsize_d')
...
if `EXIST' {
   ...
   g_spmap_psl, l(`hfcolor') m(`NG') o({bf:{ul:hfc}olor()}) d(`hfcolor_d')
   local hfcolor `"`s(pl)'"'
}
...
if `EXIST' {
   ...
   g_spmap_psl, l(`hocolor') m(`NG') o({bf:{ul:hoc}olor()}) d(`hocolor_d')
   local hocolor `"`s(pl)'"'
}

/* Parse option hosize() */
g_spmap_psl, l(`hosize') m(`NG') o({bf:{ul:hos}ize()}) d(`hosize_d')
local hosize `"`s(pl)'"'
...
if `EXIST' {
   ...
   g_spmap_psl, l(`lcolor') m(`NG') o({bf:{ul:lco}lor()}) d(`lcolor_d')
   local lcolor `"`s(pl)'"'
}

/* Parse option lsize() */
g_spmap_psl, l(`lsize') m(`NG') o({bf:{ul:lsi}ze()}) d(`lsize_d')
local lsize `"`s(pl)'"'

/* Parse option lpattern() */
g_spmap_psl, l(`lpattern') m(`NG') o({bf:{ul:lpa}ttern()}) d(`lpattern_d')
local lpattern `"`s(pl)'"'
