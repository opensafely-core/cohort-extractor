...
program define llogistic, eclass byable(recall) prop(ml_score)
	...
	if replay() {
		...
		* global EREGt `t'
		...
		if "`ancillary'" == "" {
			local dip diparm(`lngam', exp label(gamma))
			if _caller() >= 15 {
		...
		ml model `mlmetho' logis_lf /*
			...
			*/ max `search' `log' `dip' `options' `negh' `moptobj'
		if "`e(wtype)'" != "" {
			est local wexp `"`exp'"'
		}
	...
	if "`coef'"=="" {
		...
		if "`header'" == "" {
			di _n as txt /*
		      */ "Log-logistic AFT regression -- entry time `e(t0)'"
		}
		version 9: ///
