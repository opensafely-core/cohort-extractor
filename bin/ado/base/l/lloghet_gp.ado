...
*/ "`xb'`index'`stdp'`time'`lntime'`hazard'`hr'`surv'`deviance'`csnell'`mgale'`csurv'`ccsnell'`cmgale'"
	...
	if "`type'"=="" | "`type'"=="time" {
		...
		tempname cc
		scalar `cc' = (expm1(((0.5)^(-`theta')-1)/`theta'))^`gamma'
...
*/ "(option {bf:median lntime} assumed; predicted median log time)"
		...
		qui _predict double `xb' if `touse', xb `offset'
		tempname cc
		scalar `cc' = expm1(((0.5)^(-`theta')-1)/`theta') 
...
*/ "proportional-hazards parameterization"
	...
	if "`type'"=="hazard" {
		...
		*/ `kk'/(`gamma'*`t'*(1+`kk')) if `touse'
		qui replace `ff'=`ff'/(1+`theta'*ln1p(`kk')) if `touse'
	...
	if "`type'"=="mgale" { 
		...
		qui gen double `ff'= /*
		*/ (`d'!=0)-(1/`theta')* /*
		*/ ln1p(`theta'*ln1p((`t'*exp(-`xb'))^(1/`gamma'))) /*
		*/ if `touse'
		qui replace /*
		*/ `ff'=`ff'+(1/`theta')* /*
		*/ ln1p(`theta'*ln1p((`t0'*exp(-`xb'))^(1/`gamma'))) /*
		*/ if `t0'>0 & `touse'
		gen `vtyp' `varn' = `ff' if `touse' 
		if "`id'"!="" { local part "partial " }
