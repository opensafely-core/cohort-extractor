...
*/ "(option {bf:median lntime} assumed; predicted median log time)"
		...
		qui _predict double `xb' if `touse', xb `offset'
		tempname cc
		scalar `cc' = `gamma'*ln(expm1(((1-`theta'*ln(0.5))^2-1)/ /*
			*/ (2*`theta')))
...
*/ "proportional-hazards parameterization"
	...
	if "`type'" =="surv" {
		tempvar xb ff ff0
		qui _predict double `xb' if `touse', xb `offset'
		qui gen double `ff'=ln1p((`t'*exp(-`xb'))^(1/`gamma')) /*
			*/ if `touse'
		qui gen double `ff0'=ln1p((`t0'*exp(-`xb'))^(1/`gamma')) /*
			*/ if `t0'>0 & `touse'
	...
	if "`type'"=="mgale" { 
		...
		*/ (`d'!=0)+(1/`theta')* /*
		*/ (1-sqrt(1+2*`theta'* /*
		*/ ln1p((`t'*exp(-`xb'))^(1/`gamma')))) if `touse'
		qui replace /*
		*/ `ff'=`ff'-(1/`theta')* /*
		*/ (1-sqrt(1+2*`theta'* /*
		*/ ln1p((`t0'*exp(-`xb'))^(1/`gamma')))) if `t0'>0 & `touse'
		gen `vtyp' `varn' = `ff' if `touse' 
		if "`id'"!="" { local part "partial " }
