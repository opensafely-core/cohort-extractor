...
program define lnorm_ic_p, sort
	version 15
...
*/ "proportional-hazards parameterization"
	...
	if "`type'"=="hazard" {
		...
		if "`lower'" != "" {
			qui gen double `ff'= 
normalden((ln(`ltime')-`xb')/`sigma')/(`ltime'*`sigma') if `touse'
			qui replace `ff'= ///
		   	`ff'/( 1-normal((ln(`ltime')-`xb')/`sigma')) ///
				if `touse'
		...
		if "`upper'" != "" {
			qui gen double `ff'= ///
normalden((ln(`rtime')-`xb')/`sigma')/(`rtime'*`sigma') if `touse'
			qui replace `ff'= ///
		   	`ff'/( 1-normal((ln(`rtime')-`xb')/`sigma')) ///
				if `touse'
			gen `vtyp' `varn'= `ff' if `touse'
...
normalden((ln(`rtime')-`xb')/`sigma')/(`rtime'*`sigma') if `touse'
	...
	if "`type'" =="surv" {
		...
		if "`lower'" != "" {
			gen `vtyp' `varn'=  ///
			    1-normal((ln(`ltime')-`xb')/`sigma') if `touse'
			label var `varn' "Predicted S(`ltime')"
			exit
		}
		if "`upper'" != "" {
			gen `vtyp' `varn' =  ///
			    1-normal((ln(`rtime')-`xb')/`sigma') if `touse'
		...
		if `nvar' == 1 {
			...
			gen `vtyp' `var2' =  ///
			    1-normal((ln(`rtime')-`xb')/`sigma') if `touse'
			label var `var2' "Predicted S(`ltime')"
	...
	if "`type'"=="csnell" {
		tempvar surv
		qui lnorm_ic_p double `surv'_L `surv'_R ///
