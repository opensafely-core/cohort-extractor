*! version 1.3.0  22mar2018
program define heckprobit, eclass byable(onecall) ///
...
program define Estimate, eclass byable(recall)
	...
	syntax [varlist(numeric default=none ts fv)] [pw iw fw] [if] [in], /*
		...
		*/ OFFset(varname numeric ts) Robust noSKIP/*undoc*/	   /*
	...
	if "`lrmodel'" != "" {
		_check_lrmodel, `skip' `constant' constr(`cns')	///
			options(`cluster' `robust') indep(`varlist')
		local skip "noskip"
	}
	else {
	...
	`qui' probit `depvar' `varlist' `wgt' if `touse' & `seldep', /*
		*/ `constant' `offopt' nocoef `crtype' `doopt' `cns1' `mllog'
	scalar `llc' = e(ll)
	...
	if "`robust'"=="" | "`from'"=="" | ("`skip'"!="" & "`from0'"=="") {
		...
		`qui' probit `seldep' `selind' `wgt' if `touse',  /*
			...
			*/ iter(`=min(1000,c(maxiter))') `cns1' `doopt' `mllog'
		mat `b0sel' = e(b)
	...
	if "`skip'" != "" & "`from0'" == "" {
		...
		`qui' probit `depvar' `one' `nshaz' `wgt' 	/*
			...
			*/ iter(`=min(1000,c(maxiter))') `doopt' `mllog'
	...
	if "`from'" == "" {
		...
		`qui' probit `depvar' `varlist' `one' `nshaz' `wgt'	/*
			*/ if `touse' & `seldep', 			/*
			*/ noconstant `offopt' nocoef asis `crtype'	/*
			*/ iter(`=min(1000,c(maxiter))') `doopt' `cns1' `mllog'
		MkB0 `b0' : `b0sel' `nshaz'
...
capture noisily break {
	...
	*/ `crittyp' `diparm'  userinfo(`inits') `moptobj' `mllog'
	local rc = _rc
	capture mata: rmexternal("`inits'")
	...
	if `rc' == 1400 & "`from'" == "`b0', copy" {
		...
		*/ `diparm' `crittyp' userinfo(`inits') `moptobj'
	} 
} // capture noisily break	
	local rc = _rc
	capture mata: rmexternal("`inits'")
	...
	`vv' qui _diparm athrho, tanh
	est scalar rho = r(est)
	if `tempsel' {
		est local depvar `depvar'
	}
	est scalar k_aux = 1
	est hidden scalar N_unc = e(N) - `N_cens'
	...
	est hidden local marginsnomarkout PSel XBSel
	est local marginsok default PMargin p11 p10 p01 p00 PSel PCond xb XBSel
	est local marginsnotok stdp STDPSel SCores
	if ("`selname'" != "select") {
