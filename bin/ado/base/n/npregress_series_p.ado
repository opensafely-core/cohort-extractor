*! version 1.0.2  10jan2020

program define npregress_series_p, eclass 
	version 16
	...
	local cvarscheck "`cvars'"
	tempname newmm3
	matrix `newmm3' = e(matmm3)
	if ("`e(lnointeract)'"!="" & `estis'>1) {
		local nointervars "`e(lnointeract)'"
		fvexpand `cvars'
		local cvarscheck "`r(varlist)'"
		local cvarsinit "`r(varlist)'"
		local cvarscheck: list cvarscheck - nointervars
		if ("`cvarscheck'"!="") {
			_mm3_new, orig(`cvarsinit') check(`cvarscheck') 
			matrix `newmm3' = r(newmat)
		}
	}
	local rck: list sizeof cvarscheck
	if (`estis'>1 & "`cvars'"!="" & "`cvarscheck'"!="") {
		_check_cvars if `touse3', cvars(`cvarscheck') 		///
					  tolerance(`tolerance')	///
					  newmat(`newmm3')		///
					  `atsample'
		noisily capture checkestimationsample
		local rc = _rc 
		if ((`rc'| "`c(marginsatvars)'"!="") & `rck'>1) {
			local oldvars "`e(checknoms)'"
...
program define _making_bsknots_p, rclass
	...
	local esat = 0 
	_verify_dinter, known(`e(known)') atnom(`c(marginsatvars)')
	local verk = r(verk)
	if ("`e(cmd_loco)'"=="_npreg_series_two" & "`dinter'"!="" & ///
		"`c(marginsatvars)'"!="" & `noatc'==0 & `verk'==0) {
		fvexpand `dinter'
		local dinter "`r(varlist)'"
		quietly mata: _dinter_expand_("`dinter'")
		local original  "`e(catnames)' `dinter'"
...
program define _check_intersect, rclass
	...
	marksample touse 
	tempname knots mgn A B C D orig mm sbla
	tempvar uno new 
	...
	matrix `B' = `orig''
	matrix `C' = `B'
	mata: st_numscalar("`sbla'", sum(st_matrix("`C'")))
	matrix colnames `B' = `dinter'
	local todobien = 0 
	if (`sbla') {
		mata: st_local("bla", ///
		invtokens(select(st_matrixcolstripe("`B'")[.,2], ///
			st_matrix("`C'")')'))
		...
		forvalues i=1/`k' {
			...
			forvalues j=1/`k2' {
				...
				(`nuevo' - `min1')/(`max1'-`min1')  ///
					if `touse'
				quietly generate byte ///
					`cond`i'`j'' =	///
					cond(`nuevof'<=`max', 1, 0)* ///
					cond(`nuevof'>=`min', 1, 0)  ///
					if `touse'
				capture drop `nuevof'
				if (`j'==1) {
					quietly generate `check`i'' ///
						=	///
						`cond`i'`j'' ///
						if `touse'
				}
				else {
					quietly replace `check`i'' ///
						=	///
						`check`i''* ///
						`cond`i'`j'' if `touse'
				}
			...
			if (`media'>0) {
				...
				mata: st_numscalar("e(empty_cell)", ///
				1, "hidden")
			}
		}
	}
...
program define _check_cvars
	syntax [anything] [if] [in], [cvars(string) tolerance(string)	///
					newmat(string) atsample *]
	
	marksample touse
	tempname minmax
	
	matrix `minmax' = `newmat'
	local k: list sizeof cvars	
	forvalues i=1/`k' {
		...
		local max   = r(max) 
		local min   = r(min) 
		local b0 = max(abs(`minmax'[`i',2]), abs(`minmax'[`i',1]))
		local b = (`b0' + (epsdouble())^(1/5))*(epsdouble())^(1/5)
		if ("`tolerance'"=="") {
			local tolerancia = `b'
		}
		else {
...
program define _mm3_new, rclass 
	syntax [anything], [orig(string) check(string)]
	
	tempname newmat oldmat
	local k1: list sizeof orig
	local k2: list sizeof check
	matrix `oldmat' = e(matmm3)
	
	forvalues i=1/`k1' {
		local x: word `i' of `orig'
		forvalues j=1/`k2' {
			local y: word `j' of `check'
			if ("`x'"=="`y'") {
				matrix `newmat' = ///
				(nullmat(`newmat') \ (`oldmat'[`i', 1..2]))
			}
		}
	}
	return matrix newmat = `newmat'
end

program _verify_dinter, rclass
	syntax [anything], [known(string) atnom(string)]
	
	local verk = 0 
	if ("`known'"!="" & "`atnom'"!="") {
		fvexpand `known'
		local lknown "`r(varlist)'"
		local k1: list sizeof known
		local lknownf ""
		forvalues i=1/`k1' {
			local x: word `i' of `lknown'
			_ms_parse_parts `x'
			if ("`r(k_names)'"!="") {
				local k2 = r(k_names)
				forvalues j = 1/`k2' {
					local lknownf "`lknownf' `r(name`j')'"
				}
			}
			else {
				local lknownf "`lknownf' `r(name)'"
			}		
		}
		local lknonwf: list uniq lknownf
		fvexpand `atnom'
		local latnom "`r(varlist)'"
		_ms_parse_parts `latnom'
		local latnomf "`r(name)'"
		local inter: list latnomf & lknownf
		if ("`inter'"!="") {
			local verk = 1 
		}
	}
	return local verk = `verk'
end
