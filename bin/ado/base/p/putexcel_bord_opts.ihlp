...
is {cmd:thin}.  For a complete list of border styles, see the
{help putexcel##style:{it:Appendix}} in {bf:[RPT] putexcel}.
To remove an existing border, specify {cmd:none} as the {it:style}.
...
or may be a valid RGB value in the form {cmd:"}{it:### ### ###}{cmd:"}.
If no {it:color} is specified, then Excel workbook defaults are used.
{p_end}
