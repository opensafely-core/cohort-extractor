...
{p2colset 1 21 23 2}{...}
{p2col:{bf:[ME] meqrpoisson} {hline 2}}Multilevel mixed-effects Poisson
regression (QR decomposition){p_end}
{p2col:}({browse "http://www.stata.com/manuals15/memeqrpoisson.pdf":View complete PDF manual entry}){p_end}
{p2colreset}{...}
...
{pstd}
{cmd:meqrpoisson} is a legacy command for fitting mixed-effects models to
binary or binomial responses.  {cmd:mepoisson} is the modern command, and it
offers more functionality; see {manhelp mepoisson ME}. The two commands use
different but equivalent estimation methods. {cmd:mepoisson} performs
optimization using variance components in their original metric, whereas
{cmd:meqrpoisson} uses the QR decomposition of the variance-components matrix.
