...
program mgarch_dvech, sortpreserve eclass byable(recall)
	...
	local maxopts "`log' `nolog' `trace' `gradient' `hessian' `showstep' "
...
//	local last_ob  = r(lasts)
	...
	forvalues j=1/`k' {
		forvalues i=`j'/`k' {
			local nm "Sigma0:`i'_`j'"
			if `version' >= 16 {
				local nm "/`nm'"
			}
			local vnames `"`vnames' `nm'"'
		}	
	}
	...
	if (`estdf') {
		if `version' < 16 {
			...
			local nm "/:`dfname'"
			local delta_names `"`delta_names' `nm'"'
			local cvnames `"`cvnames' `nm'"'
		}
	}
