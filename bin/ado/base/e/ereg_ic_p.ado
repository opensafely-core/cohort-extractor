...
program define ereg_ic_p, sort
	...
	if `"`scores'"' != "" {
		...
		*/
	local myopts ///
"XB STDP Time LNTime HAzard HR Surv CSnell MGale OOS MEDian MEAN"
	...
	*/ "(option {bf:median lntime} assumed; predicted median log time)"
	...
	if "`type'"=="lntime" & "`median'"=="median" {
		tempvar elnt
		qui ereg_ic_p double `elnt' if `touse', time median
	...
	if "`type'"=="hazard" {
		...
		if `nvar' == 1 {
			...
			local var1 `1'
			local var2 `2'
			gen `vtyp' `var1'=exp(-`sgn'*`xb') ///
					if `touse' & `ltime'!=.
			gen `vtyp' `var2'=exp(-`sgn'*`xb') ///
					if `touse' & `rtime'!=.
	...
	if "`type'" =="surv" {
		...
		if `nvar' == 1 {
			...
			*/ if `touse'
			...
			gen `vtyp' `var1' = /*
			*/ exp( -exp(-`sgn'*`xb')*`ltime' ) if `touse'
			gen `vtyp' `var2' = /*
			*/ exp( -exp(-`sgn'*`xb')*`rtime' ) if `touse'
	...
	if "`type'"=="csnell" {
		tempvar surv
		qui ereg_ic_p double `surv'_L `surv'_R if `touse', surv `offset'
