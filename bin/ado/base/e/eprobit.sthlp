...
{viewerjumpto "Syntax" "eprobit##syntax"}{...}
{viewerjumpto "Menu" "eprobit##menu"}{...}
{viewerjumpto "Description" "eprobit##description"}{...}
{viewerjumpto "Links to PDF documentation" "eprobit##linkspdf"}{...}
{viewerjumpto "Options" "eprobit##options"}{...}
{viewerjumpto "Examples" "eprobit##examples"}{...}
{viewerjumpto "Stored results" "eprobit##results"}{...}
{p2colset 1 18 20 2}{...}
{p2col:{bf:[ERM] eprobit} {hline 2}}Extended probit regression{p_end}
{p2col:}({mansection ERM eprobit:View complete PDF manual entry}){p_end}
...
{pstd}
Probit regression combining random effects, endogenous covariates, treatment,
and selection
...
{synoptline}
{syntab :Model}
{synopt :{opth endog:enous(eprobit##enspec:enspec)}}model for endogenous covariates; may be repeated{p_end}
{synopt :{opth entr:eat(eprobit##entrspec:entrspec)}}model for endogenous treatment assignment{p_end}
{synopt :{opth extr:eat(eprobit##extrspec:extrspec)}}exogenous treatment{p_end}
{synopt :{opth sel:ect(eprobit##selspec:selspec)}}probit model for selection{p_end}
{synopt :{opth tobitsel:ect(eprobit##tselspec:tselspec)}}tobit model for selection{p_end}
{synoptline}
...
{synopt :{it:{help eprobit##display_options:display_options}}}control
...
{syntab :Maximization}
{synopt :{it:{help eprobit##maximize_options:maximize_options}}}control the maximization process; seldom used{p_end}
...
{p 4 6 2}
{opt nore} is available only with {cmd:xteprobit}.
...
are allowed with {cmd:eprobit} and {cmd:xteprobit}.
{cmd:rolling} and {cmd:svy} are allowed with {cmd:eprobit}.
...
allowed with {cmd:eprobit}; see {help weight}.{p_end}
{p 4 6 2}
...
{p 4 6 2}
See {manhelp eprobit_postestimation ERM:eprobit postestimation} for features
...
{pstd}
{cmd:eprobit} fits a probit regression model that accommodates any combination
of endogenous covariates, nonrandom treatment assignment, and endogenous
sample selection.  Continuous, binary, and ordinal endogenous covariates are
allowed.  Treatment assignment may be endogenous or exogenous.  A probit or
tobit model may be used to account for endogenous sample selection. 

{pstd}
{cmd:xteprobit} fits a random-effects probit regression model that
accommodates endogenous covariates, treatment, and sample selection in the
same way as {cmd:eprobit} and also accounts for correlation of observations
...
{title:Links to PDF documentation}
        ...
        {mansection ERM eprobitQuickstart:Quick start}

        {mansection ERM eprobitRemarksandexamples:Remarks and examples}

        {mansection ERM eprobitMethodsandformulas:Methods and formulas}
...
{pmore}
The default technique for {cmd:eprobit} is {cmd:technique(nr)}.  The default
technique for {cmd:xteprobit} is {cmd:technique(bhhh 10 nr 2)}.
...
{pstd}
The following options are available with {opt eprobit} and {opt xteprobit} but
...
{pstd}
Probit regression with endogenous covariate {cmd:hsgpa}{p_end}
{phang2}
...
{pstd}
As above, and account for endogenous treatment{p_end}
{phang2}
{cmd:. eprobit graduate income i.roommate, endogenous(hsgpa = income i.hscomp)}
...
{pstd}
Probit regression with endogenous sample selection{p_end}
{phang2}
{cmd:. eprobit attack age bmi i.exercise, select(full = age bmi i.checkup)}

{pstd}
As above, and account for endogeneity of the {cmd: exercise} treatment{p_end}
{phang2}
...
{pstd}
Random-effects probit regression{p_end}
{phang2}
{cmd:. xteprobit goodhlth exercise grade}

{pstd}
Random-effects probit with endogenous treatment{p_end}
{phang2}
{cmd:. xteprobit goodhlth exercise grade, entreat(insured = grade i.workschool)}

{pstd}
Random-effects probit with endogenous sample selection{p_end}
{phang2}
...
{pstd}
{cmd:eprobit} stores the following in {cmd:e()}:
...
{pstd}
{cmd:xteprobit} stores the following in {cmd:e()}:
...
{p2col 5 20 24 2:Macros}{p_end}
{synopt:{cmd:e(cmd)}}{cmd:xteprobit}{p_end}
{synopt:{cmd:e(cmdline)}}command as typed{p_end}
{synopt:{cmd:e(depvar)}}names of dependent variables{p_end}
{synopt:{cmd:e(tsel_ll)}}left-censoring limit for tobit selection{p_end}
{synopt:{cmd:e(tsel_ul)}}right-censoring limit for tobit selection{p_end}
