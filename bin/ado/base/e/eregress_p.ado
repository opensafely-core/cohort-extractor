*! version 1.0.5  13sep2019
program define eregress_p
	version 15
	...
	if "`structural'" != "" {
		...
		if "`outlevel'" != "" {
			opts_exclusive "structural outlevel()"
		}
		if "`scores'" != "" {
		...
		if "`expmean'" != "" {
			opts_exclusive "structural expmean"
		}	 		
	}
	else {
		local total total
		if "`pr'`tet'`scores'`cond'`pre'`e'`ystar'`expmean'" ///
			== "" & ("`total'" == "" & 		///
			"`equation'" == "`e(odepvar)'") {
	...
	local linearendog `e(odepvar)' `e(endogdepvars)' `e(tseldepvar)'
	local inlinear: list linearendog & equation
	local allstat `xb'`pr'`pomean'`te'`tet'`mean'
	local allstat `allstat'`e'`scores'`ystar'`pre'`expmean'
	if "`inlinear'" != "" & "`allstat'" == "" {
		local mean mean
		...
		local pr pr
		di as text  "(option {bf:pr} assumed; predicted probabilities)"
	}
	local opttest
	if "`outlevel'" != "" {
	...
	if "`scores'" != "" {
		...
		if "`e(cmd)'" == "eregress" | 		///
			("`e(cmd)'" == "xteregress" |	///
			 "`e(cmd)'" == "xtheckman") {		
			_eregress_get_scores `0'
		}
		else {
			_eintreg_get_scores `0'
		}
		exit 0
	}
	if "`xb'" != "" {
		local opttest xb `opttest' `mean' `pr' ///
			`pomean' `te' `tet' `expmean'
		local w: word count `opttest'
		if `w' > 1 {
			opts_exclusive "`opttest'"
		}
		...
		_predict `anything' `if' `in', xb `equation' 
		exit
	}
	local opttest
	if "`outlevel'" != "" {
	...
	if "`pr'" != "" {
		local opttest pr `pomean' `te' `tet' `expmean'
		local w: word count `opttest'
		if `w' > 1 {
			opts_exclusive "`opttest'"
		}
	...
	if "`pomean'" != "" {
		local opttest pomean `te' `tet' `expmean'
		local w: word count `opttest'
		if `w' > 1 {
			opts_exclusive "`opttest'"
		}
	...
	if "`outlevel'" != "" {
		...
		if "`expmean'" != "" {
			di as err "{bf:outlevel()} incorrectly " ///
				"specified;"
			di as err "{p 0 4 2}an outcome value "	
			di as err "may not be specified with {bf:expmean}"	
			di as err "{p_end}"
			exit 498
		}
	...
	else if "`expmean'`mean'`e'`pre'`ystar'" != "" {
		_stubstar2names `anything', nvars(1)
		local varlist `s(varlist)'
		local typlist `s(typlist)'		
	}
		...
		if "`equation'" == "`e(odepvar)'" {
			if "`fix'" != "" {
				...
				if "`e(treatrecursive)'"!="" & "`te'" != "" {
					...
					di as error "{bf:tet} instead{p_end}"
					exit 498
				}
				if "`e(treatrecursive)'"!="" & "`pomean'"!="" {
					...
					di as error "the treatment, try "
					di as error " {bf:mean()}"
					di as error " and {bf:target()}{p_end}"
					exit 498
				}
	...
	if "`meanasf'" != "" {
		_stubstar2names `anything', nvars(1)
		local varlist `s(varlist)'
		_predict `anything' `if' `in', xb equation(`e(odepvar)') 
		if "`e(depvar_dvlist)'" != "" {
			label variable `varlist' ///
				"Structural mean of `e(odepvar)'"
		}
		else {
			label variable `varlist' "mean of `e(odepvar)'"
		}
		exit
	}
	if "`pomean'" != "" {
		...
		if `tetetalltm1' {
			...
			forvalues i = 1/`ntm1' {
				...
				local ltlevel: value label ///
					`e(extrdepvar)'`e(trdepvar)'
				capture label list `ltlevel'
				if _rc {
					local ltlevel 
				}
				if "`ltlevel'" != "" {
					local ltlevel: ///
						label `ltlevel' `tlevel'
				}
				else {
					local ltlevel `tlevel'
				}				
				local lclevel: value label ///
					`e(extrdepvar)'`e(trdepvar)'
				capture label list `lclevel'
				if _rc {
					local lclevel 
				}
				if "`lclevel'" != "" {
				...
				if "`tet'" != "" {
					local lab treatment effect ///
						on the treated, ///
			`e(extrdepvar)'`e(trdepvar)': `ltlevel' vs. `lclevel'
				}
				else {
					local lab treatment effect, ///
			`e(extrdepvar)'`e(trdepvar)': `ltlevel' vs. `lclevel'
		...
		if "`inlinear'"  == "" {
			noi di as error 	///
			"{bf:expmean} only allowed with a linear outcome"
			exit 498
		}
		_eregress_expmean, varn(`varlist')		/// 
			vart(`typlist')				///
			eq(`equation') cond(`cond')		/// 
			touse(`touse')				///
			intpoints(`intpoints')			/// 
