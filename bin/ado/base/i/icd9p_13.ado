...
* this is -icd9p- for caller versions less than 14
program define icd9p_13
	version 9
	...
	if "`cmd'"=="check" { 
		...
		di in red "invalid icd9p subcommand"
		exit 198
	}
end
...
program define Lookup
	...
	qui icd9p_13 clean __code9, dots
...
program define Tabulate
	...
	quietly {
		...
		keep `2' `wgtv' `c'
		icd9p_13 gen `desc' = `c', desc long end
		label var `desc' `"`lbl'"'
	}
