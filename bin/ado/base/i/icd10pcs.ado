*! version 1.0.10  06dec2019
program icd10pcs
	version 15
	gettoken cmd 0 : 0, parse(" ,")
	local l = length(`"`cmd'"')

	global icd10pcs_year = 2020
	...
	if `"`cmd'"' == "check" {
		...
		di as err "icd10pcs subcommand required"
		exit 198
	}
	else {
		di as err "invalid icd10pcs subcommand"
		exit 198
	}
end

// icd10pcs check
program icd10pcs_check, rclass
	...
	if "`version'" != "" {
		icd10pcs_check_version "`version'"
		local vn `"intro(__intro) del(__del) version(`version')"'
	}
	else {
		local vn `"intro(__intro) del(__del) version($icd10pcs_year)"'
		local version "$icd10pcs_year"
	}
...
di as err `"may not specify both option {bf:summary} and option {bf:list}"'
	...
	tempvar prob
	noi icd check `varlist' `if' `in', using("__icd10pcs")		///
	...
	if `mymiss' == `Nuse' {
		...
		quietly {
			...
			// 5.  1st char (0-9, A-H, J-N, P-Z)
			replace `prob'= 5 if `c'!="" 			///
				& indexnot(`c',c(ALPHA))		///
				& (`c'<"0" | `c'>"9")			///
				& (`probeq') & `touse'
			replace `prob'=5 if (`c' == "I" | `c' == "O")	///
				& (`probeq') & `touse'
			count if `prob'==5 & `touse'
			local mye5 = r(N)

			//  6.  2nd char (0-9, A-H, J-N, P-Z)
			replace `c' = bsubstr(`code',2,1) if `touse'
			replace `prob'=6 if `c'!="" 			///
				& indexnot(`c',c(ALPHA))		///
				& (`c'<"0" | `c'>"9")			///
				& (`probeq') & `touse'
			replace `prob'=6 if (`c' == "I" | `c' == "O")	///
				& (`probeq') & `touse'
			count if `prob'==6 & `touse'
			local mye6 = r(N)

			// 7.  3rd char (0-9, A-H, J-N, P-Z)
			replace `c' = bsubstr(`code',3,1) if `touse'
			replace `prob'=7 if `c'!="" 			///
				& indexnot(`c',c(ALPHA))		///
				& (`c'<"0" | `c'>"9")			///
				& (`probeq') & `touse'
			replace `prob'=7 if (`c' == "I" | `c' == "O")	///
				& (`probeq') & `touse'
			count if `prob'==7 & `touse'
			local mye7 = r(N)

			// 8.  4th char (0-9, A-H, J-N, P-Z)
			...
			replace `prob'=8 if `c'!="" 			///
				...
				& (`c'<"0" | `c'>"9")			///
				& (`probeq') & `touse'
			replace `prob'=8 if (`c' == "I" | `c' == "O")	///
				& (`probeq') & `touse'
			count if `prob'== 8 & `touse'
			local mye8 = r(N)

			// 9.  5th char (0-9, A-H, J-N, P-Z)
			...
			replace `prob'=9 if `c'!="" 			///
				...
				& (`c'<"0" | `c'>"9")			///
				& (`probeq') & `touse'
			replace `prob'=9 if (`c' == "I" | `c' == "O")	///
				& (`probeq') & `touse'
			count if `prob'== 9 & `touse'
			local mye9 = r(N)

			// 10.  6th char (0-9, A-H, J-N, P-Z)
			...
			replace `prob'=10 if `c'!="" 			///
				...
				& (`c'<"0" | `c'>"9")			///
				& (`probeq') & `touse'
			replace `prob'=10 if (`c' == "I" | `c' == "O")	///
				& (`probeq') & `touse'
			count if `prob'== 10 & `touse'
			local mye10 = r(N)

			// 11.  7th char (0-9, A-H, J-N, P-Z)
			replace `c' = bsubstr(`code',7,1) if `touse'
			replace `prob' = 11 if `c'!="" 			///
				& indexnot(`c',c(ALPHA))		///
				& (`c'<"0" | `c'>"9")			///
				& (`probeq') & `touse'
			replace `prob'=11 if (`c' == "I" | `c' == "O")	///
				& (`probeq') & `touse'
...
// Code 1 ---------------------------------------------------------------
			di as text "    1.  Invalid placement of period" ///
				_col(52) as res %11.0gc `mye1'
			ret scalar e1 = `mye1'
// Code 2 ---------------------------------------------------------------
			di as text "    2.  Too many periods"		///
				_col(52) as res %11.0gc `mye2'
			ret scalar e2 = `mye2'
// Code 3 ---------------------------------------------------------------
			di as text "    3.  Code too short"		///
				_col(52) as res %11.0gc `mye3'
			ret scalar e3 = `mye3'
// Code 4 ---------------------------------------------------------------
			di as txt "    4.  Code too long"		///
				_col(52) as res %11.0gc `mye4'
			ret scalar e4 = `mye4'
// Code 5 ---------------------------------------------------------------
	di as txt "    5.  Invalid 1st char (not 0-9, A-H, J-N, P-Z)"	///
				_col(52) as res %11.0gc `mye5'
			ret scalar e5 = `mye5'
// Code 6 ---------------------------------------------------------------
	di as txt "    6.  Invalid 2nd char (not 0-9, A-H, J-N, P-Z)"	///
				_col(52) as res %11.0gc `mye6'
			ret scalar e6 = `mye6'
// Code7 ---------------------------------------------------------------
	di as txt "    7.  Invalid 3rd char (not 0-9, A-H, J-N, P-Z)"	///
				_col(52) as res %11.0gc `mye7'
			ret scalar e7 = `mye7'
// Code8 ---------------------------------------------------------------
	di as txt "    8.  Invalid 4rd char (not 0-9, A-H, J-N, P-Z)"	///
				_col(52) as res %11.0gc `mye8'
			ret scalar e8 = `mye8'
// Code9 ---------------------------------------------------------------
	di as txt "    9.  Invalid 5th char (not 0-9, A-H, J-N, P-Z)"	///
				_col(52) as res %11.0gc `mye9'
			ret scalar e9 = `mye9'
// Code10 ---------------------------------------------------------------
	di as txt "   10.  Invalid 6th char (not 0-9, A-H, J-N, P-Z)"	///
				_col(52) as res %11.0gc `mye10'
			ret scalar e10 = `mye10'

// Code11 ---------------------------------------------------------------
	di as txt "   11.  Invalid 7th char (not 0-9, A-H, J-N, P-Z)"	///
				_col(52) as res %11.0gc `mye11'
...
// Code 77 ---------------------------------------------------------------
				...
				"   77.  Valid only for previous versions" ///
					_col(52) as res %11.0gc `mye77'
				ret scalar e77 = `mye77'
// Code 88 ---------------------------------------------------------------
				di as text				///
				"   88.  Valid only for later versions" ///
					_col(52) as res %11.0gc `mye88'
...
// Code 99 ---------------------------------------------------------------
			...
			di in smcl as text _col(52) "{hline 11}"
			di as text _col(9) "Total" ///
				_col(52) as res %11.0gc `bad_codes'
		}
	...
	if `"`generate'"' != "" {
		...
		qui label define __icdpcs				///
			0  "Defined code"				///
			1  "Invalid placement of period"		///
			2  "Too many periods"				///
			3  "Code too short"				///
			4  "Code too long"				///
			5  "Invalid 1st char (not 0-9, A-H, J-N, P-Z)"	///
			6  "Invalid 2rd char (not 0-9, A-H, J-N, P-Z)"	///
			7  "Invalid 3rd char (not 0-9, A-H, J-N, P-Z)"	///
			8  "Invalid 4th char (not 0-9, A-H, J-N, P-Z)"	///
			9  "Invalid 5th char (not 0-9, A-H, J-N, P-Z)"	///
			10 "Invalid 6th char (not 0-9, A-H, J-N, P-Z)"	///
			11 "Invalid 7th char (not 0-9, A-H, J-N, P-Z)"	///
			77 "Valid only for previous versions"		///
			88 "Valid only for later versions"		///
			99 "Code not defined"				///
			, replace
		qui label values `generate' __icdpcs
		qui label var `generate' "result of check for `varlist'"
	}
end

//icd10pcs clean
program icd10pcs_clean
	syntax varname [if] [in] [, replace GENerate(string)		///
		nodots check						///
		]

	if "`check'" != "" {
		icd10pcs_check `varlist' `if' `in', system fmtonly
	}
	...
	icd clean `varlist' `if' `in', `maxlength' `replace' `gen' `dots' 
end

// icd10pcs generate
program icd10pcs_generate, rclass
...
di as err "option {bf:version} cannot be combined with option {bf:range()}"
		...
		local version `"intro(__intro) del(__del) version($icd10pcs_year)"'
	}
...
di as err "option {bf:dots} only allowed with option {bf:description}"
	...
	if "`description'" != "" {
		local description `"description using("__icd10pcs") `version'"'
		if "`long'" != "" {
	...
	icd generate `newvar' = `varlist' `if' `in',			///
		...
		`addcode' `long' `rules' `dots'
end

// icd10pcs lookup
program icd10pcs_lookup, rclass
	...
	icd lookup `anything', using("__icd10pcs") `vn'
	ret scalar N_codes = r(N_codes)
end

// icd10pcs search
program icd10pcs_search, rclass
	...
	icd search `anything', using("__icd10pcs") descrvar(__ldescr)	///
		`vn' `or' `matchcase'
	ret scalar N_codes = r(N_codes)
end

// icd10pcs query
program icd10pcs_query
	syntax

	icd query, using("__icd10pcs")
end

// utility commands
program icd10pcs_check_version
	args version
