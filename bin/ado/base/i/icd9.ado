...
program icd9
	...
	if `"`cmd'"'=="check" {
		...
		// -icd9 search- keeps same features as before -- drop
		// through to original command
		icd9_13 `cmd' `0'
	}
	else if `"`cmd'"' == "table" |					///
			`"`cmd'"' == bsubstr("tabulate",1,max(3,`l')) {
		// -icd9 table- and -icd9 tabulate- were not documented (in
		...
		di as err "icd9 subcommand required"
		exit 198
	}
	else {
		di as err "invalid icd9 subcommand"
		exit 198
	}
end
...
program Check, rclass
	...
	if bsubstr("`typ'",1,3)!="str" {
		di as err ///
			"`varlist' does not contain ICD-9 codes; " ///
	...
	quietly {
		...
		replace `l' = substr(`c',5,1) if `touse'
		replace `prob' = 9 if (`l'<"0" | `l'>"9") & `l'!="" ///
						& `prob'==. & `touse'
		drop `l'
		...
		gen byte `l' = length(`c') if `touse'
		replace `prob' = 3 if bsubstr(`c',1,1)=="E" & `l'<4 ///
						& `prob'==. & `touse'
	...
	if "`system'"!="" {
		...
		drop `c'
		di as err "`varlist' contains invalid ICD-9 codes"
		exit 459
	}
	...
	if `bad'==0 {
		if `miss'==`Nuse' {
			...
			di as text	///
			"(`varlist' contains defined ICD-9-CM codes; `msgstub')"
		}
		ret scalar e1 = 0
		ret scalar e2 = 0
		ret scalar e3 = 0
		...
		di as text "    5.  Invalid 1st char (not 0-9, E, or V)" ///
					_col(49) as res %11.0gc r(N)
		ret scalar e5 = r(N)
		...
		qui count if `prob'==9 & `touse'
		di as text "    9.  Invalid 5th char (not 0-9)" ///
					_col(49) as res %11.0gc r(N)
		ret scalar e9 = r(N)
...
program Clean
	...
	quietly {
		...
		if "`pad'"!="" {
			replace `c' = " " + `c' if bsubstr(`c',1,1)!="E" &`touse'
...
program GenDesc
	...
	quietly {
		...
		if "`long'" != "" {
			...
			replace `c' = " " + `c' if bsubstr(`c',1,1)!="E"&`touse'
			replace `c' = substr(`c'+"       ",1,7) if `touse'
			if "`end'"=="" {
...
program IsEl, sclass
	...
	if `"`c'"' == "" {
		di as err "<nothing> invalid ICD-9 code"
		exit 198
	}
	if strpos(`"`c'"', ".") {
		local l = strpos(`"`c'"', ".")
