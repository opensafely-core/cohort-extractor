{* *! version 1.0.2  10may2017}{...}
    {cmd:rcauchy(}{it:a}{cmd:,}{it:b}{cmd:)}
{p2colset 8 22 22 2}{...}
{p2col: Description:}Cauchy({it:a},{it:b}) random variates, where {it:a} is the
location parameter and {it:b} is the scale parameter{p_end}
{p2col: Domain {it:a}:}-1e+300 to 1e+300{p_end}
{p2col: Domain {it:b}:}1e-100 to 1e+300{p_end}
{p2col: Range:}{cmd:c(mindouble)} to {cmd:c(maxdouble)}{p_end}
{p2colreset}{...}
