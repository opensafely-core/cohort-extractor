...
{viewerjumpto "Description" "dta_119##description"}{...}
{viewerjumpto "Remarks" "dta_119##remarks"}{...}
{p2colset 1 26 28 2}{...}
...
{pstd}
The format described here went into effect as of Stata 15 and is
known as {cmd:.dta} format 119.  It is used only for datasets with
more than 32,767 variables.  The primary format used by Stata 15 and 16,
for datasets with 32,767 variables or less, is 118, which is also the
format used by Stata 14.  For documentation on format 118,
see {help dta}.  For documentation on earlier file formats,
see {help dta_117}.

{pstd}
{error:We will highlight in red changes between .dta formats 118 and 119.}
...
what are known as {cmd:.dta} format-118 and format-119 files and can read
all formats of files that have ever been released.  The recent history of
{cmd:.dta} formats is

	Format        Current as of
	{hline 39}
          119     Stata 15 and 16 (when dataset has more than 32,767 variables)
          {help dta:118}     Stata 14, 15, and 16
...
{marker f119}{...}
{pstd}
Format 119 is documented below.

{pstd}
Remarks are presented under the following headings:

	{help dta_119##intro:1.  Introduction}
	{help dta_119##versions:2.  Versions and flavors of Stata}
	{help dta_119##strings:3.  Representation of strings}
	{help dta_119##numbers:4.  Representation of numbers}
 	{help dta_119##definition:5.  Dataset format definition}
	    {help dta_119##header:5.1  Header}
                 {help dta_119##header_release:5.1.1  File format id}
                 {help dta_119##header_byteorder:5.1.2  Byteorder}
                 {help dta_119##header_K:5.1.3  K, # of variables}
                 {help dta_119##header_N:5.1.4  N, # of observations}
                 {help dta_119##header_label:5.1.5  Dataset label}
                 {help dta_119##header_timestamp:5.1.6  Datetime stamp}
	    {help dta_119##map:5.2  Map}
	    {help dta_119##variabletypes:5.3  Variable types}
	    {help dta_119##variablenames:5.4  Variable names}
	    {help dta_119##sortlist:5.5  Sort order of observations}
	    {help dta_119##formats:5.6  Display formats}
	    {help dta_119##labelnames:5.7  Value-label names}
	    {help dta_119##variablelabels:5.8  Variable labels}
	    {help dta_119##characteristics:5.9  Characteristics}
	    {help dta_119##data:5.10 Data}
            {help dta_119##strls:5.11 StrLs}
	         {help dta_119##strls_vo:5.11.1 (v,o) values}
	         {help dta_119##strls_gso:5.11.2 GSOs}
	         {help dta_119##strls_advw:5.11.3 Advice on writing strLs}
	         {help dta_119##strls_advwb:5.11.4 Advice on writing 3- and 5-byte integers}
	         {help dta_119##strls_advr:5.11.5 Advice on reading strLs}
	         {help dta_119##strls_advrb:5.11.6 Advice on reading 3- and 5-byte integers}
	    {help dta_119##value_labels:5.12 Value labels}
...
what are known as {it:format-119} datasets, one of the two most modern formats.
Stata itself can read older formats, but whenever it writes a dataset, it
writes in 118 format (if there are 32,767 variables or less) or 119 format
(if there are more than 32,767 variables).

{pstd}
There are currently three flavors of Stata available:
{help stataic:Stata/IC}, {help SpecialEdition:Stata/SE}, and
{help statamp:Stata/MP}.  All of them use 118 format, and Stata/MP
uses 119 format if there are more than 32,767 variables.
...
{pstd}
{bf:Strings are encoded UTF-8 in Stata}.  We are referring to all
...
{phang}
3.  We discuss {cmd:strL} format strings in section {help dta_119##strls:5.11}.
...
a dataset written on 10 March 2017 at 2:23 p.m. would look like this:
	...
	<stata_dta>
		...
		<label>{rm:{it:000b}}Sample Data</label>
...
actually run together, so it looks more like this,
        ...
        {hline 40} {it:top of file} {hline 5}
	<stata_dta><header><release>{red:119}</release><byteorder>MSF</by
	teorder><K>{red:{rm:{it:00000002}}}</K><N>{rm:{it:0000000000000001}}</N><label>{rm:{it:000b}}Sa
	mple Data</label><timestamp>{rm:{it:11}}10 Mar 2017 14:23</timestamp>
	</header><map>{it:0000000000000000}{it:0000000000000099}{it:0000000000000}
	{it:141}{it:0000000000000139}{it:0000000000000190}{it:00000000000001ab}{it:00000000}
	{it:00000220}{it:000000000000034e}{it:0000000000000371}{it:0000000000000384}{it:000}
	{it:0000000000393}{it:00000000000003b0}{it:00000000000003bc}</map><variabl
	e_types>{rm:{it:fff7}}{rm:{it:fff9}}</variable_types><varnames>myfloat{rm:{it:00}}.......
	.................myint{rm:{it:00}}..........................</varname
	s><sortlist>{rm:{it:00000000}}</sortlist><formats>%9.0g{rm:{it:00}}............
	...................%8.0g{rm:{it:00}}...............................</
	formats><value_label_names>{rm:{it:00}}..............................
	{rm:..{it:0}}{rm:{it:0}}................................</value_label_names><va
	riable_labels>{rm:{it:00}}...........................................
	......{rm:{it:00}}................................................</v
...
{pstd}
A 119-format {cmd:.dta} file begins with {cmd:<stata_dta>} and 
...
{pstd}
where {it:K} = {it:bbbb} is a 4-byte unsigned integer field
recorded according to {it:byteorder}.
{red:In format 118, K was written in a 2-byte field.}
...
{pstd}
where {it:sortlist} is a sequence (array) of {it:K}+1 unsigned {red:4-byte}
integers recorded according to {it:byteorder}.  

{pstd}
{it:sortlist} specifies the sort-order of the dataset and is terminated by a
{red:4-byte} zero ({red:{it:00000000}} in hex).  Each {red:4-byte} element
contains either a variable number or zero.  The zero marks the end of the
{it:sortlist}, and the recorded positions after that contain random junk.
For instance, if the data are not sorted, the first {cmd:4-byte} integer will
contain a zero, and the {cmd:4-byte} integers thereafter will contain junk.
If {cmd:nvar}==4, the record will appear as
...
appear as
	...
	{red:{cmd:0000000200000000............}}  (if {cmd:byteorder}==MSF)
	{red:{cmd:0200000000000000............}}  (if {cmd:byteorder}==LSF)
...
as
	...
	{red:{cmd:000000020000000100000000........}}  (if {cmd:byteorder}==MSF)
	{red:{cmd:020000000100000000000000........}}  (if {cmd:byteorder}==LSF)


{pstd}
If {it:sortlist} were read into the C-array {red:{cmd:int}}
...
as given by {it:typlist} and defined in sections {help dta_119##strings:3} (strfs)
and
{help dta_119##numbers:4} (numeric values).
We have not yet explained how strLs are written; we will do that in 
section {help dta_119##strls:5.11}.  In the meantime, let us imagine a dataset without strLs.
...
{pstd}
In section {help dta_119##data:5.10} we had an example showing the
...
Run together in the {cmd:.dta} file, it looks like this,
	...
	<data>{it:00000000014000000000000000}first{it:00}{red:{it:0000050000000001}}
	{it:3f800000024008000000000000}second{red:{it:0000050000000002}}</data>
        <strls>GSO{it:00050000000000018200000006}third{it:00}GSO{it:000000000}
	{it:0000005}{it:000000028200000007}fourth{it:00}</strls>
...
{p 12 12 2}
    ...
    {it:000005000000000001} and {it:000005000000000002}.

{p 12 12 2}
    {it:000005000000000001} and {it:000005000000000002} each represent an 8-byte
    field.  Interpret that 8-byte field as a 3-byte integer followed by a 
    5-byte integer:
...
{p 12 12 2}
    ...
    table, we will talk a little about this 3-byte plus 5-byte encoding of 
    ({it:v},{it:o}) in {bf:<data>}...{bf:</data>}. 

{p 12 12 2}
    The use of a 5-byte integer is awfully odd.  In the previous 117 {cmd:.dta}
    format, the ({it:v},{it:o}) values were written as two 4-byte values.  This
    accommodated Stata's then-current maximum of roughly 32,000 variables and
    roughly 2,000,000,000 observations.

{p 12 12 2}
    In the 118 format used in modern Stata when there are 32,767 variables
    or less, the ({it:v},{it:o}) values are written as a 2-byte value followed
    by a 6-byte value to allow for more observations.

{p 12 12 2}
    Since then, Stata has learned to deal with more variables
    and {it:v} no longer can be stored in just 2 bytes in the 119 format.
    Stata allows up to 100,000 variables and up to 1 terabyte observations.
    This combination of numbers can be stored in a 3-byte integer and a 5-byte
    integer.

{p 12 12 2}
    A 4-byte integer followed by an 8-byte integer would have been the natural
    choice.  But, for our own reasons, we needed this field to still be 8 bytes
    in length.  So we split it into 3 bytes plus 5 bytes, and that's adequate
    for our purposes.  
See {help dta_119##strls_advwb:5.11.4} 
and {help dta_119##strls_advrb:5.11.6} 
for C code for writing and reading 3- and 5-byte integers.
...
{p 12 12 2}
    ...
    the GSO for (5,2). 
    This time, there is no 3-byte, 5-byte silliness.  {it:v} is recorded as 
...
2.  ({it:v},{it:o}) values are recorded in an 8-byte field and are 
    {red:interpreted as a 3-byte unsigned integer followed by}
    {red:a 5-byte unsigned integer}.
...
The format of a GSO record is 
		     ...
		     o               len
                      \             /        contents
                       -------- ----         /
		{cmd:GSO}{it:vvvvooooooootllllxxxxxxxxxxxxxxx...x}
                   ----        -    [--- len bytes ---]
		  /            |
                 v           type
...
1.  {it:v} and {it:o} are the ({it:v},{it:o}) values defined in 
    ...
    the {help dta_119##vo_rules:rules} of specification previously given. 
	...
	for (j=1; j<=N; j++) {c -(}
		for (i=1; i<=K; i++) {c -(}
			if (variable i is strL) {c -(}
				if (contents of i != "") {c -(}
					write i as 3 bytes  (see {help dta_119##strls_advwb:5.11.4})
					write j as 5 bytes  (see {help dta_119##strls_advwb:5.11.4})
...
{marker strls_advwb}{...}
{title:5.11.4  Advice on writing 3- and 5-byte integers}


    #define Int4   int      /* or whatever is the type of a 4-byte integer  */
    #define Int8   longlong /* or whatever is the type of an 8-byte integer */

    #define MSF_Start_of_Int3_in_Int4(p)	(((char *) (p))+1)
    #define LSF_Start_of_Int3_in_Int4(p)	((char *) (p))
    #define MSF_Start_of_Int5_in_Int8(p)	(((char *) (p))+3)
    #define LSF_Start_of_Int5_in_Int8(p)	((char *) (p))


    /* code for byteorder MSF */
    void MSF_insert_vo_byteorder(char s[/*[8]*/], Int4 v, Int8 o)
    {
	    memcpy(s,   MSF_Start_of_Int3_in_Int4(&v), 3) ; 
	    memcpy(s+3, MSF_Start_of_Int5_in_Int8(&o), 5) ; 
    }
    

    /* code for byteorder LSF */
    void LSF_insert_vo_byteorder(char s[/*[8]*/], Int4 v, Int8 o)
    {
	    memcpy(s,   LSF_Start_of_Int3_in_Int4(&v), 3) ; 
	    memcpy(s+3, LSF_Start_of_Int5_in_Int8(&o), 5) ; 
    }
...
{title:5.11.6  Advice on reading 6-byte integers}
    ...
    /*
        The following #defines are the same as in {help dta_119##strls_advwb:5.11.4} 
    */
    ...
    */
    ...
    /* code for byteorder MSF */
    void MSF_extract_vo_byteorder(Int4 *v_ptr, Int8 *o_ptr, char s[/*[8]*/]) 
    {
            *v_ptr = 0 ; 
            memcpy(MSF_Start_of_Int3_in_Int4(o_ptr), s,   3) ; 

            *o_ptr = 0 ; 
            memcpy(MSF_Start_of_Int5_in_Int8(o_ptr), s+3, 5) ; 
    }


    /* code for byteorder LSF */
    void LSF_extract_vo_byteorder(Int4 *v_ptr, Int8 *o_ptr, char s[/*[8]*/]) 
    {
            *v_ptr = 0 ; 
            memcpy(LSF_Start_of_Int3_in_Int4(o_ptr), s,   3) ; 

            *o_ptr = 0 ; 
            memcpy(LSF_Start_of_Int5_in_Int8(o_ptr), s+3, 5) ; 
    }
...
{help dta_119##labelnames:5.7}.  Variable {cmd:sex} might be associated with 
...
where {it:def} is 
	...
	{it:labname}                129   char       max 32 UTF-8 characters, 
                                                 \0 terminated
	padding                  3
