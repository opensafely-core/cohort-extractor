...
`RetcodeS' read_header(`FileinfoS' fi, `FilecntsS' fc)
{
	...
	if (fc.rel != `RELEASE') return(err_msg("release not 118"))
					...
					`RC' varno, `RC' offset)
{
	...
	for (ci=1; ci<=rows(varno); ci++) {
		...
		pragma unset b1
		pragma unset b0
		b5 = bufget(C, buf, offset[ci]+2, "%1bu")
