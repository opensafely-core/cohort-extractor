...
/*
	This ado-file is suitable for use, unmodified, in Stata 15 
	and forward.
*/


local cmdname "dtaverify_119"
program `cmdname'
	version 13
	local 0 `"using `0'"'
	syntax using/
...
local RELEASE		119
local MaxVarlabLen	320	/* sic */	/* excludes \0 */
local MaxNameLen	128	/* sic */	/* excludes \0 */
local MaxFmtLen		56	/* sic */	/* excludes \0 */
local MaxCharLen	67784			/* INCLUDES \0 */
local MaxVallabCntsLen	32001			/* INCLUDES \0 */
local MaxVar		120000
local MaxObs 		1099511627775
...
`RetcodeS' read_header(`FileinfoS' fi, `FilecntsS' fc)
{
	...
	if (fc.rel != `RELEASE') return(err_msg("release not `RELEASE'"))
...
`RetcodeS' read_header_K(`FileinfoS' fi, `FilecntsS' fc)
{
	...
	pragma unset K
	if ((rc = filebufget(K, C, fi.h, "%4bu"))) return(rc) 
...
`RetcodeS' read_header_label(`FileinfoS' fi, `FilecntsS' fc)
{
	...
	bufbyteorder(C, fc.bufbyteorder)
	pragma unset text
	if ((rc=read_generic_string(fc, "label", fi.h, 2, text))) return(rc) 
...
`RetcodeS' read_header_timestamp(`FileinfoS' fi, `FilecntsS' fc)
{
	...
	pragma unset text
	if ((rc = read_generic_string(fc, text, fi.h, 1, text))) return(rc)
	l    = strlen(text)
...
`RetcodeS' read_sortorder(`FileinfoS' fi, `FilecntsS' fc)
{
	...
	for (i=1; i<=fc.K+1; i++) {
		pragma unset val
		if ((rc = filebufget(val, C, fi.h, "%4bu"))) return(rc)
		order[i] = val
	}
	...
	if (last== -1) {
		(void) err_msg("sortlist not 2*(Int4) 00000000 terminated")
		fc.serious = `True' 
					...
					`RC' varno, `RC' offset)
{
	...
	for (ci=1; ci<=rows(varno); ci++) {
		...
		pragma unset b1
		pragma unset b0
		b2 = bufget(C, buf, offset[ci]+0, "%1bu")
		b1 = bufget(C, buf, offset[ci]+1, "%1bu")
		b0 = bufget(C, buf, offset[ci]+2, "%1bu")
		...
		/*
			1.0x+10 = 2^16 because 0x10 = 16
			1.0x+08 = 2^8  because 0x08 =  8
		*/
		...
		pragma unset b1
		pragma unset b0
		b4 = bufget(C, buf, offset[ci]+3, "%1bu")
		b3 = bufget(C, buf, offset[ci]+4, "%1bu")
		b2 = bufget(C, buf, offset[ci]+5, "%1bu")
		b1 = bufget(C, buf, offset[ci]+6, "%1bu")
		b0 = bufget(C, buf, offset[ci]+7, "%1bu")
...
/* 
	...
	GSOdef:
            ...
            o            8          unsigned 8-byte integer, o of (v,o)
...
`RetcodeS' read_generic_string(`FilecntsS' fc, `SS' what, `Fildes' h,
						`RS' lenlen, `SS' buf)
{
	`RetcodeS'	rc
	`SS'		C
	...
	if (lenlen==1) {
		...
		if ((rc = filebufget(l, C, h, "%2bu"))) return(rc) 
	}
	else {
		err_msg(what + " incorrect length specified")
		return(`SERIOUS')
	}
